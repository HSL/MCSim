...
/* ----------------------------------------------------------------------------
   ...
   Note that here StdDevLogNorm is sigma in log space -- this convention
			   ...
			   double dStdDevLogNorm)
{
  ...
  if (dMean < 0) { /* "True value must be >= 0 */
    ...
    if (dStdDevLogNorm <= 0) {
      char str[10];
      sprintf(str, "%5.2e", dStdDevLogNorm);
      ReportRunTimeError(NULL, RE_BADLOGNORMALSD | RE_FATAL,
                         "", str, "GenLogNormalRandom");
    }

  /* Relative Standard Deviation of Lognormal */
  dSLogNorm = sqrt(exp(pow(dStdDevLogNorm,2)) * 
		   (exp(pow(dStdDevLogNorm,2)) - 1)); 
  /* Transformation parameter */
  dLambda = pow(dStdDevNorm/dSLogNorm,2);
...
/* ----------------------------------------------------------------------------
   ...
   degrees of freedom, location dMean, and scale dStdDev.
*/
...
double StudentTRandom (double dof, double dMean, double dStdDev)
{
  ...
  return (dMean + dStdDev*z*sqrt(dof/x));
